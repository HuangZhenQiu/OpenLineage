...
class IcebergSinkVisitorTest {
  ...
  void testApply() {
    ...
    try (MockedStatic<IcebergSinkWrapper> mockedStatic = mockStatic(IcebergSinkWrapper.class)) {
      ...
      when(wrapper.getNamespace()).thenReturn(Optional.of("catalog"));
      ...
      Assertions.assertEquals(Constants.ICEBERG_TYPE, symlinkIdentifier.getType());
      Assertions.assertEquals("table", symlinkIdentifier.getName());
      Assertions.assertEquals("catalog", symlinkIdentifier.getNamespace());
    }
  }
}
